// ==========================================
// ENGINE.JS - Lógica de Negócios e Cálculos (Otimizado v28.5)
// ==========================================
...
function showToast(mensagem, tipo = 'sucesso') {
    ...
    else if (tipo === 'alerta') { corFundo = '#f59e0b'; icone = 'fa-exclamation-triangle'; }
    ...
    container.style.cssText = 'position: fixed; top: 20px; left: 50%; transform: translateX(-50%); z-index: 999999; display: flex; flex-direction: column; align-items: center;';
...
window.preencherSelectContasSalario = function() {
    ...
    const contasValidas = db.contas.filter(c => c.tipo !== 'cartao');
    
    if (contasValidas.length === 0) {
        selectConta.innerHTML = '<option value="">Cadastre uma conta bancária primeiro</option>';
        return;
    }
    contasValidas.forEach(c => { selectConta.innerHTML += `<option value="${c.id}">🏦 ${c.nome}</option>`; });
};
...
window.exportarExtratoCSV = function() {
    ...
    let csvContent = "data:text/csv;charset=utf-8,Data;Descrição;Categoria;Conta/Cartão;Direção;Status;Valor\n";
    exportData.forEach(l => {
        ...
        const valorBR = (parseFloat(l.valor) || 0).toFixed(2).replace('.', ',');
        csvContent += `${dataPT};"${l.desc}";"${l.cat}";"${nomeConta}";${direcao};${status};${valorBR}\n`;
    });
    const link = document.createElement("a"); link.setAttribute("href", encodeURI(csvContent)); link.setAttribute("download", `EcoFinance_Relatorio_${mesFiltro}.csv`); document.body.appendChild(link); link.click(); link.remove(); showToast("Relatório exportado!");
}
...
window.verificarNotificacoesMotor = function() {
    ...
    let htmlNotificacoes = ''; let contadorUnread = 0;
    const hoje = new Date(); const diaHoje = hoje.getDate(); const dataHojeIso = hoje.toISOString().split('T')[0];
    ...
    (db.lancamentos || []).forEach(l => {
        ...
        if (l.data && !l.efetivado && window.ecoTiposDespesa.includes(l.tipo)) {
            ...
            const diasFaltando = Math.round((d - hoje) / 86400000);
            if (diasFaltando <= 3) {
                contadorUnread++;
                ...
                <div class="notif-card" style="background: var(--input-bg); border: 1px solid ${diasFaltando < 0 ? 'var(--perigo)' : 'var(--alerta)'}; padding: 12px; border-radius: 10px; margin-bottom: 10px;">
                    ...
                    <button class="btn-outline" style="width: 100%; font-size: 11px; padding: 8px;" onclick="irParaExtratoEMes('${l.data.substring(0,7)}', ${l.id})">Ir para Extrato</button>
                </div>`;
            }
        }
    });
    ...
    (db.contas || []).filter(c => c.tipo === 'cartao').forEach(c => {
        ...
        (db.lancamentos || []).forEach(l => {
            if(l.contaId === c.id) {
                const m = window.obterMesFaturaAviso(l.data, c.fechamento || 1);
                meses[m] = (meses[m]||0) + (window.ecoTiposReceita.includes(l.tipo) ? -l.valor : l.valor);
            }
        });
        Object.keys(meses).forEach(m => {
            const fatID = `${c.id}-${m}`;
            if ((db.faturasPagas || []).includes(fatID)) return;
            const t = meses[m] - ((db.amortizacoesFaturas && db.amortizacoesFaturas[fatID]) || 0);
            if (t <= 0) return;
            
            let [a, ms] = m.split('-'); 
            if ((c.vencimento||1) < (c.fechamento||1)) { ms = parseInt(ms)+1; if(ms>12){ms=1;a=parseInt(a)+1;} }
            const dv = new Date(a, ms-1, c.vencimento||1); dv.setHours(0,0,0,0);
            const diasFaltando = Math.round((dv - hoje) / 86400000);
            
            if (diasFaltando <= 3) {
                contadorUnread++;
                let statusAviso = diasFaltando < 0 ? '🚨 Fatura Atrasada' : (diasFaltando === 0 ? '⚠️ Fatura Vence Hoje' : `⚠️ Fatura em ${diasFaltando} dias`);
                htmlNotificacoes += `
                <div class="notif-card" style="background: var(--input-bg); border: 1px solid ${diasFaltando < 0 ? 'var(--perigo)' : 'var(--alerta)'}; padding: 12px; border-radius: 10px; margin-bottom: 10px;">
                    <strong style="font-size: 13px; color: var(--texto-main); display:block; margin-bottom: 8px;">${statusAviso}</strong>
                    <p style="font-size: 12px; color: var(--texto-sec); margin-bottom: 12px;">A fatura do cartão <strong>${c.nome}</strong> (R$ ${t.toFixed(2)}) precisa de atenção.</p>
                    <button class="btn-outline" style="width: 100%; font-size: 11px; padding: 8px;" onclick="if(typeof window.acionarVerFaturaCtx === 'function') window.acionarVerFaturaCtx(null, '${c.id}', '${m}');">Ver Fatura</button>
                </div>`;
            }
        });
    });

    if(htmlNotificacoes === '') htmlNotificacoes = '<div style="text-align:center; padding: 20px; color: var(--texto-sec); font-size: 12px;"><i class="fas fa-check-circle" style="font-size: 24px; color: var(--esmeralda); margin-bottom: 10px; display:block;"></i>Tudo em dia! Nenhuma pendência próxima.</div>';
    listaNotif.innerHTML = htmlNotificacoes;
    if (contadorUnread > 0) { badge.style.display = 'flex'; badge.innerText = contadorUnread; } 
    else { badge.style.display = 'none'; }
}

window.irParaExtratoEMes = function(mesAno, highlightId) {
    if (typeof navegar === 'function') navegar('historico', document.querySelector('.menu-item[onclick*="historico"]'));
    const inputMes = document.getElementById('filtro-mes');
    if (inputMes) { inputMes.value = mesAno; if (typeof renderHistorico === 'function') renderHistorico(); }
    ...
    if (highlightId) {
        setTimeout(() => {
            const card = document.getElementById(`lancamento-card-${highlightId}`);
            if (card) {
                card.scrollIntoView({ behavior: 'smooth', block: 'center' });
                card.classList.add('fade-highlight');
                setTimeout(() => card.classList.remove('fade-highlight'), 2500);
            }
        }, 600);
    }
};
...
window.recalcularFaturaPaga = function(contaId, mesLogico) {
    const fatID = `${contaId}-${mesLogico}`;
    if (!(db.faturasPagas || []).includes(fatID)) return;
    ...
    let novoTotal = 0;
    db.lancamentos.forEach(l => {
        if (l.contaId === contaId && window.obterMesFaturaAviso(l.data, c.fechamento||1) === mesLogico) {
            if (window.ecoTiposDespesa.includes(l.tipo)) novoTotal += parseFloat(l.valor) || 0;
            else if (window.ecoTiposReceita.includes(l.tipo)) novoTotal -= parseFloat(l.valor) || 0;
        }
    });
    
    const jaAmortizado = (db.amortizacoesFaturas && db.amortizacoesFaturas[fatID]) || 0;
    novoTotal -= jaAmortizado;
    if(novoTotal < 0) novoTotal = 0;
    
    let lancPg = db.lancamentos.find(l => String(l.id) === `pg_fat_${fatID}`);
    if (!lancPg) {
        const parts = fatID.split('-');
        const meses = {'01':'Jan', '02':'Fev', '03':'Mar', '04':'Abr', '05':'Mai', '06':'Jun', '07':'Jul', '08':'Ago', '09':'Set', '10':'Out', '11':'Nov', '12':'Dez'};
        lancPg = db.lancamentos.find(l => l.desc && l.desc.includes('Pagamento Fatura') && l.desc.includes(parts[1]) && l.desc.includes(meses[parts[2]]));
    }

    if (lancPg) {
        const diferenca = novoTotal - parseFloat(lancPg.valor);
        const contaBanco = db.contas.find(x => x.id === lancPg.contaId);
        if (contaBanco) contaBanco.saldo -= diferenca; 
        lancPg.valor = novoTotal;
    }
    save();
};
...
window.renderGrafico = function() {
    ...
    const mesFiltro = document.getElementById('filtro-mes') ? document.getElementById('filtro-mes').value : new Date().toISOString().substring(0,7);
    let totaisPorCat = {}; let temDados = false;
    ...
    (db.lancamentos || []).forEach(l => {
        ...
        if (conta && conta.tipo === 'cartao') mesRef = window.obterMesFaturaAviso(l.data, conta.fechamento || 1);
        if (mesRef === mesFiltro && window.ecoTiposDespesa.includes(l.tipo) && !['emprestei_cartao', 'emp_cartao', 'dep_poupanca'].includes(l.tipo)) {
            const catName = l.cat || 'Outros'; totaisPorCat[catName] = (totaisPorCat[catName] || 0) + valorLimpo; temDados = true;
        }
    });

    if(!temDados) {
        ctx.parentNode.style.display = 'flex'; ctx.parentNode.style.justifyContent = 'center'; ctx.parentNode.style.alignItems = 'center';
    ...
    const bgColors = labels.map(nome => { const cDB = (db.categorias || []).find(c => c.nome === nome); return cDB ? cDB.cor : '#94a3b8'; });
    ...
    window.meuGrafico = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                datalabels: { color: '#fff', font: { family: 'Inter', weight: 'bold', size: 10 }, formatter: function(value) { return 'R$ ' + value.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2}); }, display: function(context) { return context.dataset.data[context.dataIndex] > 0; } }
            }
        }
    });
};
...
window.renderGraficoEvolucao = function() {
    ...
    if (selectAno) {
        if (selectAno.options.length === 0) {
            let anos = new Set(); anos.add(anoAtual);
            (db.lancamentos || []).forEach(l => { if(l.data) anos.add(parseInt(l.data.split('-')[0])); });
            Array.from(anos).sort((a,b)=>b-a).forEach(a => selectAno.options.add(new Option(a, a))); selectAno.value = anoAtual;
        } else anoAtual = parseInt(selectAno.value) || anoAtual;
    }
    ...
    window.meuGraficoEvolucao = new Chart(ctx, {
        type: 'bar',
        data: { labels: mesesLabels, datasets: [ { label: 'Receitas', data: dadosRec, backgroundColor: '#10b981', borderRadius: 4, barPercentage: 0.6, categoryPercentage: 0.8 }, { label: 'Despesas', data: dadosDesp, backgroundColor: '#ef4444', borderRadius: 4, barPercentage: 0.6, categoryPercentage: 0.8 } ] },
        options: {
            responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false },
            scales: { x: { grid: { display: false }, ticks: { color: colorText, font: { family: 'Inter', size: 10 } } }, y: { grid: { color: gridColor }, border: { display: false }, ticks: { color: colorText, font: { family: 'Inter', size: 10 }, callback: function(value) { return 'R$ ' + (value >= 1000 ? (value/1000).toFixed(1) + 'k' : value); } } } },
            plugins: { legend: { position: 'top', align: 'end', labels: { usePointStyle: true, boxWidth: 8, color: colorText, font: { family: 'Inter', size: 11 } } }, tooltip: { backgroundColor: isDark ? 'rgba(15, 23, 42, 0.9)' : 'rgba(255, 255, 255, 0.9)', titleColor: isDark ? '#fff' : '#000', bodyColor: isDark ? '#fff' : '#000', borderColor: isDark ? '#334155' : '#cbd5e1', borderWidth: 1, padding: 10, callbacks: { label: function(context) { return context.dataset.label + ': R$ ' + context.raw.toFixed(2).replace('.', ','); } } }, datalabels: { display: false } }
        }
    });
};
...
function atualizarRegrasLancamento() {
    const direcaoSelect = document.getElementById('lanc-direcao'); const tipoSelect = document.getElementById('lanc-tipo');
    const contaSelect = document.getElementById('lanc-conta'); const boxFixo = document.getElementById('lanc-fixo');
    const catChipsContainer = document.getElementById('lanc-cat-chips'); const catInputHidden = document.getElementById('lanc-cat');
    if(!tipoSelect || !contaSelect || !catChipsContainer || !catInputHidden || !boxFixo) return;

    const tipo = tipoSelect.value; const direcao = direcaoSelect.value;
    contaSelect.options.length = 0; catChipsContainer.innerHTML = '';

    let listaCategorias = [];
    if (db.categorias && db.categorias.length > 0) {
        listaCategorias = db.categorias.filter(c => c.tipo === direcao).map(c => ({ val: c.nome, txt: `${c.icone || ''} ${c.nome}`.trim() }));
    }
    if (listaCategorias.length === 0) listaCategorias.push({ val: 'Outros', txt: '⚙️ Outros' });

    if (window.innerWidth >= 768) {
        const sel = document.createElement('select'); sel.className = 'input-moderno'; sel.style.width = '100%';
        listaCategorias.forEach(cat => sel.options.add(new Option(cat.txt.replace(/&nbsp;/g, ' '), cat.val)));
        sel.onchange = function() { catInputHidden.value = this.value; };
        catChipsContainer.appendChild(sel); catInputHidden.value = sel.value;
    } else {
        listaCategorias.forEach((cat, index) => {
            const btn = document.createElement('button'); btn.className = `chip-cat ${index === 0 ? 'active' : ''}`; btn.innerHTML = cat.txt;
            btn.onclick = function(e) { e.preventDefault(); document.querySelectorAll('#lanc-cat-chips .chip-cat').forEach(c => c.classList.remove('active')); this.classList.add('active'); catInputHidden.value = cat.val; };
...
window.adicionarLancamento = function() {
    ...
    let recalcFatID = null;
    if (conta && conta.tipo === 'cartao') {
        const mesFaturaLanc = window.obterMesFaturaAviso(data, conta.fechamento || 1);
        if ((db.faturasPagas || []).includes(`${conta.id}-${mesFaturaLanc}`)) {
            recalcFatID = { conta: conta.id, mes: mesFaturaLanc };
        }
    }
    ...
    if (tipo === 'emprestei_cartao') {
        ...
        transacoesCriadas.forEach((t, idx) => {
            ...
            let vencFatMes = mFat; let vencFatAno = aFat;
            ...
            db.lancamentos.push({
                ...
                valor: t.valor,
                cat: 'Terceiros',
                efetivado: false, // Sempre pendente (espera confirmação)
...
window.confirmarPagamentoParcial = function() {
    ...
    save(); if (typeof fecharModalParcial === 'function') fecharModalParcial(); showToast("Amortização registrada!", "ajuste"); 
    if(typeof render === 'function') render(); 
    if(typeof renderHistorico === 'function') renderHistorico();
    verificarNotificacoesMotor();
}
...
window.motorPagarFatura = function(fatID, valorTotal) {
    if ((db.faturasPagas || []).includes(fatID)) return alert("Esta fatura já consta como paga.");
    
    if (typeof window.alternarPagamentoFatura === 'function') window.alternarPagamentoFatura(fatID, valorTotal);
    else {
...
window.confirmarPagamentoFaturaBanco = function() {
    ...
    db.faturasPagas.push(fatID); save();
...
window.motorEstornarFatura = function(fatID) {
    ...
    if (db.faturasPagas) db.faturasPagas = db.faturasPagas.filter(id => id !== fatID);
    ...
    save(); showToast("Fatura reaberta e valor estornado!", "ajuste");
    if(typeof render === 'function') render();
    if(typeof renderHistorico === 'function') renderHistorico(); 
    verificarNotificacoesMotor();
};
...
window.amortizarFatura = function(fatID) {
    const fatIDParts = fatID.split('-'); 
    const contaId = fatIDParts[0]; const anoRef = fatIDParts[1]; const mesRefStr = fatIDParts[2];
    const mesRefConcatenado = `${anoRef}-${mesRefStr}`;

    const conta = db.contas.find(c => c.id === contaId); if (!conta) return;
    
    let totalFat = 0; 
    db.lancamentos.forEach(l => { 
        if (l.contaId === conta.id && window.obterMesFaturaAviso(l.data, conta.fechamento || 1) === mesRefConcatenado) {
    ...
    if(selectAmortizar && db.contas) {
        selectAmortizar.innerHTML = '';
        let contasValidas = db.contas.filter(c => c.tipo !== 'cartao'); 
        contasValidas.forEach(c => {
            selectAmortizar.innerHTML += `<option value="${c.id}">${c.nome} (Saldo: R$ ${typeof fmtBR === 'function' ? fmtBR(c.saldo) : c.saldo.toFixed(2).replace('.', ',')})</option>`;
        });
    }
...
window.confirmarAmortizacaoFatura = function() {
    ...
    const contaSelecionadaId = document.getElementById('select-conta-amortizar').value;
    const contaCC = db.contas.find(c => c.id === contaSelecionadaId);

    if (valorAmortizar <= 0) return alert("Valor inválido.");
    if (!contaCC) return alert("Selecione a conta de origem para a amortização.");
    ...
    const fatIDParts = fatID.split('-'); const anoRef = fatIDParts[1]; const mesRefStr = fatIDParts[2];
    const meses = {'01':'Jan', '02':'Fev', '03':'Mar', '04':'Abr', '05':'Mai', '06':'Jun', '07':'Jul', '08':'Ago', '09':'Set', '10':'Out', '11':'Nov', '12':'Dez'};
    
    db.lancamentos.push({ id: `am_fat_${fatID}_${Date.now()}`, desc: `Amortização Fatura (${meses[mesRefStr]}/${anoRef})`, valor: valorAmortizar, data: new Date().toISOString().split('T')[0], contaId: contaCC.id, cat: 'Outros', tipo: 'despesa', efetivado: true });
...
window.excluirLancamento = function(id) {
    ...
    const contaLanc = (db.contas || []).find(c => c.id === lanc.contaId);
    let recalcFatID = null;
    if (contaLanc && contaLanc.tipo === 'cartao') {
        const mesFat = window.obterMesFaturaAviso(lanc.data, contaLanc.fechamento || 1);
        if ((db.faturasPagas || []).includes(`${contaLanc.id}-${mesFat}`)) recalcFatID = { conta: contaLanc.id, mes: mesFat };
    }

    const executarExclusaoNormal = () => {
        if(contaLanc && contaLanc.tipo !== 'cartao' && lanc.efetivado) { 
        ...
        // Remove a transação + remove Restituições atreladas (Empréstimos)
...
// =========================================================================
// OTIMIZAÇÃO: INJEÇÃO SEGURA DE CONTATOS (PARCELAS E FIXAS) - SOLUÇÃO PONTO 5
// =========================================================================
window.salvarParcelamentoAndamento = function() {
    ...
    for(let i = parcAtual; i <= parcTotal; i++) {
        ...
        // Injeta como pendente. Lógica do render cuidará de esconder do resumo se for futuro.
...
window.salvarContaFixa = function() {
    ...
    const hoje = new Date(); let anoAtual = hoje.getFullYear(); let mesAtual = hoje.getMonth() + 1;

    let d = new Date(anoAtual, mesAtual - 1, dia);
    if (d.getMonth() + 1 !== mesAtual) d = new Date(anoAtual, mesAtual, 0); 
    
    let dataForm = `${d.getFullYear()}-${(d.getMonth()+1).toString().padStart(2,'0')}-${d.getDate().toString().padStart(2,'0')}`;

    db.lancamentos.push({ id: Date.now() + Math.floor(Math.random()*1000), idRecorrencia: newId, data: dataForm, tipo: 'despesas_gerais', contaId: contaId, forma: forma, desc: desc, valor: valor, cat: cat, efetivado: false, fixo: true });
    save(); showToast(`Conta Fixa Registrada! (1 mês projetado)`);
...
window.salvarEdicaoLancamento = function(id) {
    ...
    if (lanc.efetivado) return alert("Bloqueado: Para evitar erros de saldo, reabra este lançamento antes de editá-lo.");

    const contaAnterior = db.contas.find(c => c.id === lanc.contaId);
    let recalcAntiga = null;
    if (contaAnterior && contaAnterior.tipo === 'cartao') recalcAntiga = { conta: contaAnterior.id, mes: window.obterMesFaturaAviso(lanc.data, contaAnterior.fechamento || 1) };

    const novaDesc = document.getElementById(`e-lanc-desc-${id}`).value.trim();
    const novaContaId = document.getElementById(`e-lanc-conta-${id}`).value;
    const novaCat = document.getElementById(`e-lanc-cat-${id}`).value;
    const novaData = document.getElementById(`e-lanc-data-${id}`).value;
    
    let elValor = document.getElementById(`e-lanc-val-${id}`).value.replace(/[^\d.,]/g, '');
    ...
    if (!novaDesc || novoValor <= 0 || !novaData || !novaContaId) return alert("Preencha todos os campos corretamente.");
    ...
    if (lancOrigem && !lancOrigem.efetivado) { lancOrigem.desc = `Restituição: ${novaDesc}`; lancOrigem.valor = novoValor; }

    const contaNova = db.contas.find(c => c.id === novaContaId);
    let recalcNova = null;
    if (contaNova && contaNova.tipo === 'cartao') recalcNova = { conta: contaNova.id, mes: window.obterMesFaturaAviso(lanc.data, contaNova.fechamento || 1) };

    save();

    if (recalcAntiga && (db.faturasPagas || []).includes(`${recalcAntiga.conta}-${recalcAntiga.mes}`)) window.recalcularFaturaPaga(recalcAntiga.conta, recalcAntiga.mes);
    if (recalcNova && (recalcNova.conta !== (recalcAntiga?recalcAntiga.conta:null) || recalcNova.mes !== (recalcAntiga?recalcAntiga.mes:null))) {
        if ((db.faturasPagas || []).includes(`${recalcNova.conta}-${recalcNova.mes}`)) window.recalcularFaturaPaga(recalcNova.conta, recalcNova.mes);
    }

    if (typeof window.toggleEditLancamento === 'function') window.toggleEditLancamento(id);
    if (typeof render === 'function') render();
    if (typeof renderHistorico === 'function') renderHistorico();
