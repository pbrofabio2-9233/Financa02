...
window.renderGrafico = function() {
    ...
    const catOrdenadas = Object.keys(categorias).sort((a,b) => categorias[b] - categorias[a]);
    const labels = catOrdenadas; const dados = catOrdenadas.map(k => categorias[k]);
    ...
    meuGrafico = new Chart(ctx, { 
        type:'bar', 
        data:{ labels, datasets:[{ data:dados, backgroundColor: bgColors, borderRadius: 6 }] }, 
        options:{ 
            indexAxis: 'y', 
            responsive:true, 
            ...
            scales: { 
                x: { display: false, suggestedMax: maxVal * 1.2 }, 
                y: { grid: { display: false }, ticks: { color: isDark ? '#cbd5e1' : '#475569', font:{weight:'bold', size:11} } } 
            }
        } 
    });
}
...
window.renderGraficoEvolucao = function() {
    ...
    if (filtroAno && filtroAno.options.length === 0) { anosArr.forEach(a => filtroAno.options.add(new Option(a, a))); filtroAno.value = anoAtual; }

    const anoSelecionado = filtroAno ? filtroAno.value : anoAtual;
    ...
    if(meuGraficoEvolucao) meuGraficoEvolucao.destroy(); const isDark = document.body.classList.contains('dark-mode') || document.body.classList.contains('ocean-mode');
    ...
    if (wrapper) setTimeout(() => wrapper.scrollLeft = (anoSelecionado === anoAtual && new Date().getMonth() > 5) ? wrapper.scrollWidth : 0, 150);
}
